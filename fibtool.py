    ...
    for row in view:
        ...
        for item in row['components']:
            if item.get('name'):
                match item['type']:
                    case 'button':
                        line.append({"button": item['name'], "text": item['text'], "onReleased": dict[item['name']]})
                    case 'slider':
                        line.append({"slider": item['name'], "text": item['text'], "onChanged": dict[item['name']]})
                    case 'label':
...
def deleteResource(id,path,params):
    print(f"Deleting {path}/{id}")
    if params.dry:
        return
    
dumpFqa = lambda data,params: dumpResource(data,params,'fqa',addTQAEheader)
dumpScene = lambda data,params: dumpResource(data,params,'scene')
dumpGV = lambda data,params: dumpResource(data,params,'gv')

deleteFqa = lambda data,params: deleteResource(data['id'],"/devices",params)
deleteScene = lambda data,params: deleteResource(data['id'],"/scenes",params)
deleteGV = lambda data,params: deleteResource(data['name'],"/globalVariables",params)
...
def getResource(path1,path2,key,args):
    if args.id:
        ...
        res = []
        for id in rsrcs:
            res.append({'rsrc':json.loads(httpGet(path2+"/"+str(id))),'keyid':str(id)})
        return res
    else:
        return []
